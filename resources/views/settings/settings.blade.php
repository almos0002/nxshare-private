        ...
        <form id="settings-form" method="POST" action="{{ route('settings.update') }}" class="space-y-6">
            ...
            <div class="overflow-hidden rounded-xl bg-white shadow-sm dark:bg-surface-800">
                <div class="border-b border-surface-200 px-6 py-4 dark:border-surface-700">
                    ...
                    <p class="mt-1 text-sm text-surface-500 dark:text-surface-400">Configure your active domain and redirect
                        preferences</p>
                </div>
                <div class="p-6 space-y-6">
                    <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                        <!-- Active Domain -->
                        <div>
                            <label for="active_domain"
                                class="block text-sm font-medium text-surface-700 dark:text-surface-300">Active
                                Domain</label>
                            <div class="mt-1">
                                <div class="relative rounded-md shadow-sm">
                                    ...
                                    <input type="url" name="active_domain" id="active_domain"
                                        value="{{ old('active_domain', $settings->active_domain) }}"
                                ...
                                <p class="mt-1.5 text-xs text-surface-500 dark:text-surface-400">Enter the full URL
                                    including http:// or https://</p>
                            </div>
                        </div>
                        ...
                        <div>
                            <label for="redirect_enabled"
                                class="block text-sm font-medium text-surface-700 dark:text-surface-300">Enable
                                Redirects</label>
                            <div class="mt-1">
                                <div class="flex items-center">
                                    <label class="relative inline-flex cursor-pointer items-center">
                                        <input type="hidden" name="redirect_enabled" value="0">
                                        <input type="checkbox" name="redirect_enabled" id="redirect_enabled" value="1"
                                            class="peer sr-only"
                                            {{ old('redirect_enabled', $settings->redirect_enabled) ? 'checked' : '' }}>
                                        <div
                                        ...
                                        <span
                                            class="ml-3 text-sm font-medium text-surface-700 dark:text-surface-300">{{ old('redirect_enabled', $settings->redirect_enabled) ? 'Enabled' : 'Disabled' }}</span>
                                    </label>
                                </div>
                                <p class="mt-1.5 text-xs text-surface-500 dark:text-surface-400">When enabled, users will be
                                    redirected to the active domain</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="overflow-hidden rounded-xl bg-white shadow-sm dark:bg-surface-800">
                <div class="border-b border-surface-200 px-6 py-4 dark:border-surface-700">
                    ...
                    <p class="mt-1 text-sm text-surface-500 dark:text-surface-400">Configure advertisement code for
                        different sections</p>
                </div>
                <div class="p-6 space-y-6">
                    <!-- Ad Section 1 -->
                    <div>
                        <label for="ad1" class="block text-sm font-medium text-surface-700 dark:text-surface-300">Ad
                            Section 1</label>
                        <div class="mt-1">
                            <textarea name="ad1" id="ad1" rows="4" placeholder="Enter HTML or JavaScript code for ad section 1"
                                class="block w-full rounded-md border-surface-300 py-2 text-surface-900 placeholder:text-surface-400 focus:border-brand-500 focus:ring-brand-500 dark:border-surface-600 dark:bg-surface-700 dark:text-white dark:placeholder:text-surface-500">{{ old('ad1', $settings->ad1) }}</textarea>
                    ...
                    <!-- Ad Section 2 -->
                    <div>
                        <label for="ad2" class="block text-sm font-medium text-surface-700 dark:text-surface-300">Ad
                            Section 2</label>
                        <div class="mt-1">
