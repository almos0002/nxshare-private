...
<body class="bg-surface-50 text-surface-900 dark:bg-surface-950 dark:text-surface-100 antialiased" 
    ...
    <div class="flex h-screen overflow-hidden">
        ...
        <div x-show="mobileMenuOpen || window.innerWidth >= 1024" 
            ...
            <!-- Logo -->
            <div class="flex h-16 items-center justify-between px-4 border-b border-surface-200 dark:border-surface-800">
                <a href="#" class="flex items-center space-x-3" :class="{ 'justify-center': sidebarCollapsed, 'w-full': sidebarCollapsed }">
                    <img src="https://i.postimg.cc/4dbDJLpG/favicon.png" alt="NxShare" class="h-8 w-8">
                    <span class="text-xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-brand-600 to-brand-400" x-show="!sidebarCollapsed">NxShare</span>
                </a>
            </div>
            
            <!-- User profile -->
            <div class="px-4 py-4 border-b border-surface-200 dark:border-surface-800" :class="{ 'px-2': sidebarCollapsed }">
                <div class="flex items-center space-x-3" :class="{ 'justify-center': sidebarCollapsed }">
                    <img src="https://i.postimg.cc/JhnVsqv9/fire.png" alt="Profile" class="h-10 w-10 rounded-full ring-2 ring-brand-500 dark:ring-brand-400">
                    <div x-show="!sidebarCollapsed">
                        <p class="font-medium text-surface-900 dark:text-white">{{ Auth::user()->name ?? 'User' }}</p>
                        <p class="text-xs text-surface-500 dark:text-surface-400">{{ Auth::user()->email ?? 'user@example.com' }}</p>
                    </div>
                </div>
                <div class="mt-4 grid grid-cols-2 gap-2" x-show="!sidebarCollapsed">
                    <a href="#" class="flex items-center justify-center px-3 py-2 text-sm font-medium rounded-lg bg-surface-100 dark:bg-surface-800 text-surface-700 dark:text-surface-300 hover:bg-surface-200 dark:hover:bg-surface-700 transition-colors">
                        <i class="ri-user-settings-line mr-2 text-brand-500 dark:text-brand-400"></i> Profile
                    </a>
                    <a href="{{ route('logout') }}" onclick="event.preventDefault(); document.getElementById('logout-form').submit();" class="flex items-center justify-center px-3 py-2 text-sm font-medium rounded-lg bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 hover:bg-red-100 dark:hover:bg-red-900/30">
            ...
            <div class="flex-1 overflow-y-auto py-4 px-3" :class="{ 'px-2': sidebarCollapsed }">
                <div class="space-y-1">
                    <p class="px-3 text-xs font-semibold uppercase tracking-wider text-surface-500 dark:text-surface-400 mb-2" x-show="!sidebarCollapsed">
                        Main
                    </p>
                    ...
                    <a href="{{route('dashboard')}}" 
                       ...
                       @mouseleave="sidebarCollapsed && $dispatch('tooltip-leave')">
                        <i class="ri-dashboard-line text-lg text-brand-500 dark:text-brand-400" :class="sidebarCollapsed ? 'mx-auto' : 'mr-3'"></i>
                    ...
                    <div x-data="{ open: {{ request()->routeIs('addwp') || request()->routeIs('addpfp') ? 'true' : 'false' }} }" class="space-y-1">
                        <button @click="open = !open" 
                            ...
                            <div class="flex items-center" :class="{ 'justify-center w-full': sidebarCollapsed }">
                                <i class="ri-bubble-chart-line text-lg text-surface-500 dark:text-surface-400" :class="sidebarCollapsed ? 'mx-auto text-xl' : 'mr-3'"></i>
                                <span x-show="!sidebarCollapsed">SFW Content</span>
                            </div>
                            <i :class="open ? 'ri-arrow-down-s-line' : 'ri-arrow-right-s-line'" class="text-lg" x-show="!sidebarCollapsed"></i>
                        </button>
                        <div x-show="open" class="space-y-1" :class="{ 'pl-0 mt-1': sidebarCollapsed, 'pl-10': !sidebarCollapsed }">
                            <a href="{{route('addwp')}}" 
                               ...
                               @mouseleave="sidebarCollapsed && $dispatch('tooltip-leave')">
                                <div class="flex items-center" :class="{ 'justify-center': sidebarCollapsed }">
                                    <i class="ri-image-line {{ request()->routeIs('addwp') ? 'text-brand-500' : 'text-surface-500' }}" :class="sidebarCollapsed ? 'mx-auto text-sm' : 'mr-2'"></i>
                            ...
                            <a href="{{route('addpfp')}}" 
                               ...
                               @mouseleave="sidebarCollapsed && $dispatch('tooltip-leave')">
                                <div class="flex items-center" :class="{ 'justify-center': sidebarCollapsed }">
                                    <i class="ri-user-line {{ request()->routeIs('addpfp') ? 'text-brand-500' : 'text-surface-500' }}" :class="sidebarCollapsed ? 'mx-auto text-sm' : 'mr-2'"></i>
                    ...
                    <div x-data="{ open: {{ request()->routeIs('addnx') || request()->routeIs('addimg') || request()->routeIs('addvd') ? 'true' : 'false' }} }" class="space-y-1" id="nsfw-menu-item" style="display: {{ Auth::check() && Auth::user()->settings && Auth::user()->settings->nsfw === 'enabled' ? 'block' : 'none' }};">
                        <button @click="open = !open" 
                                ...
                                @mouseleave="sidebarCollapsed && $dispatch('tooltip-leave')">
                            <div class="flex items-center" :class="{ 'justify-center w-full': sidebarCollapsed }">
                                <i class="ri-vip-crown-line text-lg text-red-500 dark:text-red-400" :class="sidebarCollapsed ? 'mx-auto text-xl' : 'mr-3'"></i>
                        ...
                        <div x-show="open" class="space-y-1" :class="{ 'pl-0 mt-1': sidebarCollapsed, 'pl-10': !sidebarCollapsed }">
                            <a href="{{route('addnx')}}" 
                               ...
                               @mouseleave="sidebarCollapsed && $dispatch('tooltip-leave')">
                                <div class="flex items-center" :class="{ 'justify-center': sidebarCollapsed }">
                                    <i class="ri-file-lock-line {{ request()->routeIs('addnx') ? 'text-red-500' : 'text-surface-500' }}" :class="sidebarCollapsed ? 'mx-auto text-sm' : 'mr-2'"></i>
                            ...
                            <a href="{{route('addimg')}}" 
                               ...
                               @mouseleave="sidebarCollapsed && $dispatch('tooltip-leave')">
                                <div class="flex items-center" :class="{ 'justify-center': sidebarCollapsed }">
                                    <i class="ri-image-2-line {{ request()->routeIs('addimg') ? 'text-red-500' : 'text-surface-500' }}" :class="sidebarCollapsed ? 'mx-auto text-sm' : 'mr-2'"></i>
                            ...
                            <a href="{{route('addvd')}}" 
                               ...
                               @mouseleave="sidebarCollapsed && $dispatch('tooltip-leave')">
                                <div class="flex items-center" :class="{ 'justify-center': sidebarCollapsed }">
                                    <i class="ri-video-line {{ request()->routeIs('addvd') ? 'text-red-500' : 'text-surface-500' }}" :class="sidebarCollapsed ? 'mx-auto text-sm' : 'mr-2'"></i>
                ...
                <div class="mt-8 space-y-1">
                    ...
                    <a href="{{ route('settings.edit') }}" 
                       ...
                       @mouseleave="sidebarCollapsed && $dispatch('tooltip-leave')">
                        <i class="ri-settings-3-line text-lg text-surface-500 dark:text-surface-400" :class="sidebarCollapsed ? 'mx-auto' : 'mr-3'"></i>
                    ...
                    <a href="{{ route('profile.edit') }}" 
                       ...
                       @mouseleave="sidebarCollapsed && $dispatch('tooltip-leave')">
                        <i class="ri-user-settings-line text-lg text-surface-500 dark:text-surface-400" :class="sidebarCollapsed ? 'mx-auto' : 'mr-3'"></i>
            ...
            <!-- Theme toggle -->
            <div class="p-4 border-t border-surface-200 dark:border-surface-800" :class="{ 'px-2': sidebarCollapsed }">
                        ...
                        <div class="flex items-center justify-between w-full">
                            ...
                            <div class="relative inline-flex h-6 w-11 items-center rounded-full transition-all duration-150"
                                ...
                                <div class="inline-flex h-4 w-4 transform items-center justify-center rounded-full bg-white transition-all duration-150"
                                    ...
                                    <i class="text-xs" :class="darkMode ? 'ri-moon-line text-brand-600' : 'ri-sun-line text-yellow-400'"></i>
                                </div>
                            </div>
                        </div>
                    </template>
        ...
        <div x-show="mobileSearchOpen" 
            ...
            <div class="w-full max-w-md bg-white dark:bg-surface-800 rounded-lg shadow-xl overflow-hidden">
                <div class="relative">
                    ...
                    <input type="text" placeholder="Search..." class="w-full py-3 pl-10 pr-10 bg-white dark:bg-surface-800 border-0 focus:ring-0">
                    <button @click="mobileSearchOpen = false" class="absolute inset-y-0 right-0 flex items-center pr-3">
            ...
            <header class="bg-white dark:bg-surface-900 border-b border-surface-200 dark:border-surface-800 shadow-sm">
                <div class="flex items-center justify-between h-16 px-4 md:px-6">
                    <div class="flex items-center">
                        <button @click="mobileMenuOpen = true" class="inline-flex h-10 w-10 items-center justify-center rounded-lg bg-surface-100 dark:bg-surface-800 text-surface-700 dark:text-surface-300 hover:bg-surface-200 dark:hover:bg-surface-700 transition-colors lg:hidden">
                            <i class="ri-menu-line text-xl"></i>
                        </button>
                        
                        <button @click="sidebarCollapsed = !sidebarCollapsed" class="hidden lg:inline-flex h-10 w-10 items-center justify-center rounded-lg bg-surface-100 dark:bg-surface-800 text-surface-700 dark:text-surface-300 hover:bg-surface-200 dark:hover:bg-surface-700 transition-colors ml-2">
                    ...
                    <div class="flex items-center space-x-4">
                        ...
                        <div class="hidden md:block relative">
                            ...
                            <input type="text" placeholder="Search..." class="py-2 pl-10 pr-4 w-64 rounded-lg bg-surface-100 dark:bg-surface-800 border-transparent focus:border-brand-500 focus:ring focus:ring-brand-200 dark:focus:ring-brand-800 dark:focus:border-brand-500">
                        </div>
                        
                        <!-- Mobile Search Button -->
                        <button @click="mobileSearchOpen = true" class="md:hidden inline-flex h-10 w-10 items-center justify-center rounded-lg bg-surface-100 dark:bg-surface-800 text-surface-700 dark:text-surface-300 hover:bg-surface-200 dark:hover:bg-surface-700 transition-colors">
                            <i class="ri-search-line text-xl"></i>
                        </button>
                        
                        <!-- NSFW Toggle Button -->
                        <button id="nsfw-toggle-btn" class="relative inline-flex h-10 w-10 items-center justify-center rounded-lg transition-all duration-200" 
                            ...
                            <i id="nsfw-toggle-icon" class="{{ Auth::check() && Auth::user()->settings && Auth::user()->settings->nsfw === 'enabled' ? 'ri-shield-check-fill' : 'ri-shield-flash-line' }} text-white text-lg"></i>
                            <span class="sr-only">Toggle NSFW Content</span>
                        </button>
                        
                        <!-- Notifications -->
                        <div x-data="{ open: false }" class="relative">
                            <button @click="open = !open" class="relative inline-flex h-10 w-10 items-center justify-center rounded-lg bg-surface-100 dark:bg-surface-800 text-surface-700 dark:text-surface-300 hover:bg-surface-200 dark:hover:bg-surface-700 transition-colors">
                                <i class="ri-notification-3-line text-xl"></i>
                                <span class="absolute -top-1 -right-1 h-5 w-5 flex items-center justify-center rounded-full bg-red-500 text-white text-xs font-medium">4</span>
                            </button>
                            ...
                            <div x-show="open" @click.away="open = false" x-transition 
                                 ...
                                 :class="{'right-0': window.innerWidth >= 640, 'right-[-70px] sm:right-0': window.innerWidth < 640}">
                                <div class="px-4 py-3 border-b border-surface-200 dark:border-surface-800">
                                    <div class="flex items-center justify-between">
                                        ...
                                        <span class="text-xs font-medium px-2 py-0.5 rounded-full bg-brand-100 text-brand-800 dark:bg-brand-900 dark:text-brand-200">4 New</span>
                                    </div>
                                </div>
                                
                                <div class="max-h-96 overflow-y-auto">
                                    <a href="#" class="block px-4 py-3 hover:bg-surface-50 dark:hover:bg-surface-800">
                                        <div class="flex items-start">
                                            ...
                                            <div>
                                                <p class="text-sm font-medium text-surface-900 dark:text-white">New user registered</p>
                                                <p class="text-xs text-surface-500 dark:text-surface-400 mt-1">3 minutes ago</p>
                                            </div>
                                        </div>
                                    </a>
                                        ...
                                        <div class="flex items-start">
                                            <div class="flex-shrink-0 mr-3">
                                                <div class="h-10 w-10 rounded-full bg-green-100 dark:bg-green-900 flex items-center justify-center">
                                                    <i class="ri-file-text-line text-green-600 dark:text-green-400"></i>
                                                </div>
                                            </div>
                                            <div>
                                                <p class="text-sm font-medium text-surface-900 dark:text-white">New post created</p>
                                                <p class="text-xs text-surface-500 dark:text-surface-400 mt-1">1 hour ago</p>
                                            </div>
                                        </div>
                                    </a>
                                </div>
                                ...
                                <div class="px-4 py-2 border-t border-surface-200 dark:border-surface-800">
                                    <a href="#" class="text-xs text-brand-600 dark:text-brand-400 hover:underline">View all notifications</a>
                                </div>
                            </div>
                        </div>
                    </div>
